...
use ic_cdk::management_canister::{
    self as mgmt_canister, EcdsaPublicKeyArgs, EcdsaPublicKeyResult, SchnorrPublicKeyArgs,
...
pub async fn caller_ecdsa_public_key(
    ...
    Ok((mgmt_canister::ecdsa_public_key(&arg).await?,))
}
...
pub async fn sign_with_ecdsa(
    ...
    arg.derivation_path =
        Schema::Generic.derivation_path_ending_in(&ic_cdk::api::msg_caller(), arg.derivation_path);
    Ok((mgmt_canister::sign_with_ecdsa(&arg).await?,))
}
...
pub async fn schnorr_public_key(
    ...
    Ok((mgmt_canister::schnorr_public_key(&arg).await?,))
}
...
pub async fn schnorr_sign(
    ...
    Ok((mgmt_canister::sign_with_schnorr(&arg).await?,))
}
