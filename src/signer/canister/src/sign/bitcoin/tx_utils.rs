use bitcoin::{absolute::LockTime, hashes::Hash, script::PushBytesBuf, sighash::SighashCache, transaction::Version, Address, AddressType, Amount, EcdsaSighashType, OutPoint, ScriptBuf, Sequence, Transaction, TxIn, TxOut, Txid, Witness};
...
use crate::{derivation_path::Schema, sign::{bitcoin::bitcoin_utils::transform_network, ecdsa_api::{ecdsa_pubkey_of, get_ecdsa_signature}, ecdsa_utils::sec1_to_der}};

const ECDSA_SIG_HASH_TYPE: EcdsaSighashType = EcdsaSighashType::All;

pub async fn build_p2wpkh_transaction(source_address: String, network: BitcoinNetwork, utxos_to_spend: &[Utxo], fee: u64, request_outputs: Vec<BtcTxOutput>) -> Result<Transaction, String> {
...
pub async fn btc_sign_transaction(principal: &Principal, mut transaction: Transaction, utxos: &[Utxo], source_address: String, key_name: String, network: BitcoinNetwork) -> Result<SignedTransaction, String> {
