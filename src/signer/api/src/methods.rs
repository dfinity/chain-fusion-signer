...
impl SignerMethods {
    ...
    ///
    /// For `BtcCallerSign` and `BtcCallerSend`, the work scales with the number of
    /// transaction inputs (one `sign_with_ecdsa` per input). The value returned here
    /// is the per-call overhead only; the actual fee charged is
